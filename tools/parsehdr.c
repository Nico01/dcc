...
uint8_t comment;
uint8_t hash;
uint8_t ignore1; // Special: ignore egactly 1 char
uint8_t double_slash;
uint8_t spare;

int buff_idx;
char buffer[BUFF_SIZE];

uint8_t start; /* Started recording to the buffer */
uint8_t func;  /* Function header detected */
uint8_t hash_ext;
int curly;   /* Level inside curly brackets */
int xtern;   /* Level inside a extern "C" {} situation */
int round_l; /* Level inside () */
int line, col;
uint32_t chars;
char lastch;
...
{
    printf("PH *ERROR*\nFile: %s L=%d C=%d O=%u\n%s", fileName, line, col, chars, errmsg);
}

void phWarning(const char *errmsg)
{
    printf("PH -warning-\nFile: %s L=%d C=%d O=%u\n%s\n", fileName, line, col, chars, errmsg);
}

int IsIgnore() { return (comment || quote1 || quote2 || slosh || hash || ignore1 || double_slash); }
...
{
    slosh = last_slosh = start = func = comment = double_slash = hash = ignore1 = quote1 = quote2 =
        hash_ext = false;

    buff_idx = curly = xtern = col = round_l = 0;
...
void phChar(char ch)
{
    ...
    case '(':
        if (!IsIgnore()) {
            ...
            if ((curly == xtern) && (round_l == 0) && (start)) {
                func = true;
                DBG("[FUNCTION]")
            }
            round_l++;
            sprintf(st, "[ROUND++ %d]", round_l);
            DBG(st)
        }
        break;
    ...
    default:
        if ((ch != -1) && !IsIgnore() && (curly == xtern) && (!start) && (ch != ' ')) {
            start = true;
...
bool phPost(void)
{
    ...
    if (round_l > 0) {
        sprintf(msg, "EOF: ( level = %d", round_l);
        WARN(msg);
        err = false;
    }
...
void NamesToLabel(LPPH_FUNC lpF)
{
    inti, j;
    ...
    for (i = 0; i < lpF->num_params; i++) {
        ...
        j = 0;
        while (names[i][j] != '\0') {
            if ((names[i][j] != '*') && (names[i][j] != '[') && (names[i][j] != ']')) {
...
bool MoveLastWord(char *src, char *dest) /* take arg name from type  */
{
    inti, l;
    LPSTRs2;
...
int Lookup(char *aname) /* lookup var name in labels and return arg number */
{
    inti, p;
    chartname[NAMES_L];
    intbstate = false;
...
bool Plop(char *atype, char *aname, int FAR *num_params, int FAR *num_found)
{
    charmsg[80];
    intt;
...
#define IGN ((*chp == '(') || (*chp == ')') || (*chp == ',') || (*chp == ';') || (isspace(*chp)))
...
char *p;
int indirect;
bool isLong, isShort, isUnsigned;
...
/* Add a new function to the array of function name and return types. The
    ...
    in later */
bool addNewFunc(char *name, hlType typ)
{
    int i, prev, res;
    ...
    if (numFunc >= allocFunc) {
        ...
        memset(&pFunc[allocFunc - DELTA_FUNC], 0, DELTA_FUNC * sizeof(PH_FUNC_STRUCT));
    }
...
void writeFileShort(uint16_t w)
{
    uint8_t b;
...
void saveFile(void)
{
    ...
    for (i = headFunc; i != NIL; i = pFunc[i].next) {
        ...
        writeFileShort((uint16_t)pFunc[i].typ);
        writeFileShort((uint16_t)pFunc[i].numArg);
        writeFileShort((uint16_t)pFunc[i].firstArg);
    ...
    for (i = 0; i < numArg; i++) {
        ...
        writeFileShort((uint16_t)pArg[i].typ);
    }
}
