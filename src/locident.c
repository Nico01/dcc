...
/* Creates a new stack identifier node of TYPE_BYTE_(UN)SIGN orTYPE_WORD_(UN)SIGN type.
   Returns the index to this new entry. */
...
/* Checks if the entry exists in the locSym, if so, returns the idx to this entry;
   otherwise creates a new register identifier node of type TYPE_LONG_(UN)SIGN and
   returns the index to this new entry. */
int newLongRegId(LOCAL_ID *locSym, hlType t, uint8_t regH, uint8_t regL, int ix)
{
    ...
    for (int idx = 0; idx < locSym->csym; idx++) {
        if (/*(locSym->id[idx].type == t) &&   Not checking type */
            (locSym->id[idx].id.longId.h == regH) && (locSym->id[idx].id.longId.l == regL)) {
...
/* Checks if the entry exists in the locSym, if so, returns the idx to this entry;
   otherwise creates a new global identifier node of type TYPE_LONG_(UN)SIGN and
   returns the index to this new entry. */
static int newLongGlbId(LOCAL_ID *locSym, int16_t seg, int16_t offH, int16_t offL, int ix, hlType t)
{
    ...
    for (int idx = 0; idx < locSym->csym; idx++) {
        if (/*(locSym->id[idx].type == t) &&   Not checking type */
            (locSym->id[idx].id.longGlb.seg == seg) && (locSym->id[idx].id.longGlb.offH == offH) &&
...
/* Checks if the entry exists in the locSym, if so, returns the idx to this entry;
   otherwise creates a new global identifier node of type TYPE_LONG_(UN)SIGN and
   returns the index to this new entry. */
static int newLongIdxId(LOCAL_ID *locSym, int16_t seg, int16_t offH, int16_t offL, uint8_t regi,
                        int ix, hlType t)
{
    /* Check for entry in the table */
    for (int idx = 0; idx < locSym->csym; idx++) {
...
/* Returns the index to an appropriate long identifier.
   Note: long constants should be checked first and stored
   as a long integer number in an expression record. */
...
/* Given an index into the local identifier table for a long register variable, determines
   whether regi is the high or low part, and returns the other part */
...
/* Checks if the registers regL and regH have been used independently inthe local identifier table.
   If so, macros for these registers are placed in the local identifier table,
   as these registers belong to a long register identifier.	*/
