...
/* 
   ...
   The hash tables automatically expand when they get 90% full; they arenever compressed.
...
static tableType curTableType; // Current table
...
TableInfo tableInfo[NUM_TABLE_TYPES]; // Array of info about tables
...
{
    uint16_t h = (word)(symOff ^ (symOff >> 8));
...
uint16_t findBlankSym(char *symName)
{
    ...
    printf("Could not find blank entry in table!\n"
           "Num entries is %d of %d\n", numEntry, tableSize);
...
/*
   A doubly linked list of entries belonging to the same hash bucket is maintained to prevent
   the need for many entries to be moved when deleting an entry. 
...
void deleteSym(char *symName)
{
    ...
    // No, so we just wipe out this record. Must NIL the pointer of the previous record, however
        symTab[symTab[i].prevOvf].nextOvf = NIL;
        j = i; // So we wipe out the current name
    } else {
    /* Yes, move this entry to this vacated spot. Note that the nextOvf field will still point 
       to the next record in the overflow chain, but we need to preserve the backlink for adjusting
       the current item's backlink */
...
char *addStrTbl(char *pStr)
{
    ...
    /* We can't realloc the old string table pointer, since that will potentially move 
       the string table, and pointers will be invalid.
       So we realloc this one to its present size (hopefully it won't move), and allocate a new one
    */
        allocVar((void *)pStrTab, strTabNext);
