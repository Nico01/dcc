...
static char *idxReg[8] = { "bx+si", "bx+di", "bp+si", "bp+di", "si", "di", "bp", "bx" };
...
void setRegDU(PICODE pIcode, uint8_t regi, operDu du)
{
    switch ((int)du) {
    case DEF:
...
void copyDU(PICODE pIcode, PICODE duIcode, operDu du, operDu duDu)
{
    ...
    case DEF:
        if (duDu == DEF)
            memcpy(&pIcode->du.def, &duIcode->du.def, sizeof(uint32_t));
        else
            memcpy(&pIcode->du.def, &duIcode->du.use, sizeof(uint32_t));
        break;
    case USE:
        if (duDu == DEF)
            memcpy(&pIcode->du.use, &duIcode->du.def, sizeof(uint32_t));
        else
            memcpy(&pIcode->du.use, &duIcode->du.use, sizeof(uint32_t));
        break;
    case USE_DEF:
...
{
    COND_EXPR *newExp = allocStruc(COND_EXPR);
...
{
    COND_EXPR *new = newCondExp(BOOLEAN);
...
/*
 Returns a unary conditional expression node. This procedure should only be used with the following
...
COND_EXPR *idCondExpReg(uint8_t regi, uint32_t icodeFlg, LOCAL_ID *locsym)
{
    COND_EXPR *new = newCondExp(IDENTIFIER);
    new->expr.ident.idType = REGISTER;

    if ((icodeFlg & B) || (icodeFlg & SRC_B)) {
        new->expr.ident.idNode.regiIdx = newByteWordRegId(locsym, TYPE_BYTE_SIGN, regi);
        new->expr.ident.regiType = BYTE_REG;
    } else { // word
        new->expr.ident.idNode.regiIdx = newByteWordRegId(locsym, TYPE_WORD_SIGN, regi);
...
COND_EXPR *idCondExpLoc(int off, LOCAL_ID *localId)
{
    ...
    for (i = 0; i < localId->csym; i++)
        if ((localId->id[i].id.bwId.off == off) && (localId->id[i].id.bwId.regOff == 0))
            break;
...
*/
COND_EXPR *idCondExpIdxGlob(int16_t segValue, int16_t off, uint8_t regi, LOCAL_ID *locSym)
{
    COND_EXPR *new = newCondExp(IDENTIFIER);
    ...
    for (i = 0; i < locSym->csym; i++)
        if ((locSym->id[i].id.bwGlb.seg == segValue) && (locSym->id[i].id.bwGlb.off == off) &&
...
COND_EXPR *idCondExpKte(uint32_t kte, uint8_t size)
{
    COND_EXPR *new = newCondExp(IDENTIFIER);
...
// Returns an identifier conditional expression node of type LONG_VAR
COND_EXPR *idCondExpLong(LOCAL_ID *localId, opLoc sd, PICODE pIcode, hlFirst f, int ix, operDu du, int off)
{
    COND_EXPR *new = newCondExp(IDENTIFIER);
    ...
    else { // Save it as a long expression (reg, stack or glob)
        new->expr.ident.idType = LONG_VAR;
        new->expr.ident.idNode.longIdx = newLongId(localId, sd, pIcode, f, ix, du, off);
    }
...
/*
 ...
 Temporary solution, should really be encoded as an indexed type (eg. arrays).
*/
...
COND_EXPR *idCondExpID(ID *retVal, LOCAL_ID *locsym, int ix)
{
    ...
    if (retVal->type == TYPE_LONG_SIGN) {
        idx = newLongRegId(locsym, TYPE_LONG_SIGN, retVal->id.longId.h, retVal->id.longId.l, ix);
        new->expr.ident.idType = LONG_VAR;
        new->expr.ident.idNode.longIdx = idx;
    }
    else if (retVal->type == TYPE_WORD_SIGN) {
        new->expr.ident.idType = REGISTER;
        new->expr.ident.idNode.regiIdx = newByteWordRegId(locsym, TYPE_WORD_SIGN, retVal->id.regi);
        new->expr.ident.regiType = WORD_REG;
    }
...
/*
 Returns an identifier conditional expression node, according to the given type.
 @i      : index into the icode array, used for newLongRegId only.
 @duIcode: icode instruction that needs the du set.
 @du     : operand is defined or used in current instruction.
*/
COND_EXPR *idCondExp(PICODE pIcode, opLoc sd, PPROC pProc, int i, PICODE duIcode, operDu du)
{
    COND_EXPR *new = NULL;
    ...
    {                                                                    // implicit dx:ax
        ...
        if ((pm->seg == rSS) && (pm->regi == INDEXBASE + 6)) { // idx on bp
            ...
            if (pm->off > 0) // global variable
                new = idCondExpIdxGlob(pm->segValue, pm->off, rBX, &pProc->localId);
            else
            ...
            new = idCondExpOther(pm->seg, pm->regi, pm->off);
        // TODO?: check long ops, indexed global var
    }
        ...
        if ((pm->seg == rDS) && (pm->regi > INDEXBASE + 3)) { // dereference
            switch (pm->regi) {
            case INDEXBASE + 4:
                new = idCondExpReg(rSI, 0, &pProc->localId);
                setRegDU(duIcode, rSI, du);
                break;
            case INDEXBASE + 5:
                new = idCondExpReg(rDI, 0, &pProc->localId);
                setRegDU(duIcode, rDI, du);
                break;
            case INDEXBASE + 6:
                new = idCondExpReg(rBP, 0, &pProc->localId);
                break;
            case INDEXBASE + 7:
                new = idCondExpReg(rBX, 0, &pProc->localId);
...
int hlTypeSize(COND_EXPR *exp, PPROC pproc)
{
    ...
    switch (exp->type) {
    case BOOLEAN:
        first = hlTypeSize(exp->expr.boolExpr.lhs, pproc);
    ...
    case PRE_DEC:
    case DEREFERENCE:
        return (hlTypeSize(exp->expr.unaryExp, pproc));
...
hlType expType(COND_EXPR *exp, PPROC pproc)
{
    ...
    switch (exp->type) {
    case BOOLEAN:
        first = expType(exp->expr.boolExpr.lhs, pproc);
    ...
    case PRE_DEC:
    case NEGATION:
        return expType(exp->expr.unaryExp, pproc);
        break;
...
/*
 Removes the register from the tree. If the register was part of a long register (eg. dx:ax),
 the node gets transformed into an integer register node.
*/
void removeRegFromLong(uint8_t regi, LOCAL_ID *locId, COND_EXPR *tree)
{
    ...
    case IDENTIFIER:
        ...
        if (ident->idType == LONG_VAR) {
            ...
            ident->idNode.regiIdx = newByteWordRegId(locId, TYPE_WORD_SIGN, otherRegi);
        }
        break;
    }
}
...
static char *getString(uint32_t offset)
{
    size_t len = strSize(&prog.Image[offset], '\0');
    char *str = allocMem((len * 2 + 1) * sizeof(char));
    str[0] = '"';
    str[1] = '\0';

    for (size_t i = 0; i < len; i++)
        strcat(str, cChar(prog.Image[offset + i]));
...
char *walkCondExpr(COND_EXPR *exp, PPROC pProc, int *numLoc)
{
    ...
    case IDENTIFIER:
        ...
        case LOCAL_VAR:
            sprintf(o, "%s", pProc->localId.id[exp->expr.ident.idNode.localIdx].name);
            break;
        ...
        case GLOB_VAR_IDX:
            ...
            sprintf(o, "%d[%s]", (bwGlb->seg << 4) + bwGlb->off, wordReg[bwGlb->regi - rAX]);
            break;
        ...
        case LONG_VAR:
            ...
            if (id->name[0] != '\0') // STK_FRAME & REG w/name
                ...
                appendStrTab(&cCode.decl, "%s %s; /* %s:%s */\n", hlTypes[id->type], id->name,
                             wordReg[id->id.longId.h - rAX], wordReg[id->id.longId.l - rAX]);
                sprintf(o, "%s", id->name);
                propLongId(&pProc->localId, id->id.longId.l, id->id.longId.h, id->name);
            } else { // GLB_FRAME
                if (id->id.longGlb.regi == 0) // not indexed
                    sprintf(o, "[%d]", (id->id.longGlb.seg << 4) + id->id.longGlb.offH);
                else if (id->id.longGlb.regi == rBX)
                    sprintf(o, "[%d][bx]", (id->id.longGlb.seg << 4) + id->id.longGlb.offH);
            }
            break;

        case FUNCTION:
            o = writeCall(exp->expr.ident.idNode.call.proc, exp->expr.ident.idNode.call.args, pProc,
                          numLoc);
            break;
        ...
        case OTHER:
            ...
            if (off == 0)
                sprintf(o, "%s[%s]", wordReg[exp->expr.ident.idNode.other.seg - rAX],
                        idxReg[exp->expr.ident.idNode.other.regi - INDEXBASE]);
            else if (off < 0)
                sprintf(o, "%s[%s-%s]", wordReg[exp->expr.ident.idNode.other.seg - rAX],
                        idxReg[exp->expr.ident.idNode.other.regi - INDEXBASE], hexStr(-off));
            else
                sprintf(o, "%s[%s+%s]", wordReg[exp->expr.ident.idNode.other.seg - rAX],
                        idxReg[exp->expr.ident.idNode.other.regi - INDEXBASE], hexStr(off));
        }
...
// Makes a copy of the given expression. Allocates new storage for each node. Returns the copy.
COND_EXPR *copyCondExp(COND_EXPR *exp)
{
    COND_EXPR *newExp = NULL; // Expression node copy
...
void changeBoolCondExpOp(COND_EXPR *exp, condOp newOp) { exp->expr.boolExpr.op = newOp; }

// Inserts the expression exp into the tree at the location specified by the register regi
bool insertSubTreeReg(COND_EXPR *exp, COND_EXPR **tree, uint8_t regi, LOCAL_ID *locsym)
{
    uint8_t treeReg;
    ...
    case BOOLEAN:
        if (insertSubTreeReg(exp, &(*tree)->expr.boolExpr.lhs, regi, locsym))
            return true;
        if (insertSubTreeReg(exp, &(*tree)->expr.boolExpr.rhs, regi, locsym))
            return true;
        return false;
...
// Inserts the expression exp into the tree at the location specified by the long register index longIdx
...
void freeCondExpr(COND_EXPR *exp)
{
    ...
    default: break;
    case BOOLEAN:
        freeCondExpr(exp->expr.boolExpr.lhs);
...
// Reinitalizes the expression stack (expStk) to NULL, by freeing all the space allocated (if any).
void initExpStk()
{
    EXP_STK *top;
...
{
    EXP_STK *new = allocStruc(EXP_STK);
    new->exp = exp;
    new->next = expStk;
    expStk = new;
}
...
/*
 Returns the element on the top of the local expression stack (expStk), and deallocates the space
 allocated by this node. If there are no elements on the stack, returns NULL.
*/
COND_EXPR *popExpStk()
{
    EXP_STK *top;
