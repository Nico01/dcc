...
/* Structure of the prototypes table. Same as the struct in parsehdr.h, except here
   we don't need the "next" index (the elements are already sorted by function name) */
...
// statics
uint8_t buf[100];          // A general purpose buffer
...
bool locatePattern(byte *source, Int iMin, Int iMax, byte *pattern, Int iPatLen, Int *index)
{
    Int i, j;
...
void checkStartup(STATE *pState)
{
    ...
    if (locatePattern(prog.Image, startOff, startOff + 5, pattBorl4on, sizeof(pattBorl4on), &i)) {
        ...
        if (locatePattern(prog.Image, init, init + 26, pattBorl4Init, sizeof(pattBorl4Init), &i)) {
        ...
        } else if (locatePattern(prog.Image, init, init + 26, pattBorl5Init, sizeof(pattBorl5Init),
                                 &i)) {
            ...
            goto gotVendor; /* Already have vendor */
        } else if (locatePattern(prog.Image, init, init + 26, pattBorl7Init, sizeof(pattBorl7Init),
                                 &i)) {
