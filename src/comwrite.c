...
static char *int21h[] = { "Terminate process",
...
/*
 Writes the description of the current interrupt.
 Appends it to the string s.
*/
...
void writeProcComments(PPROC p, strTable *strTab)
{
    ...
    if (p->cbParam)
        ...
        for (int i = 0; i < p->args.numArgs; i++) {
            psym = &p->args.sym[i];
    ...
    if (p->flg & PROC_ASM) {
        appendStrTab(strTab, " * Untranslatable routine.  Assembler provided.\n");
    ...
    if (p->flg & (PROC_BADINST | PROC_IJMP))
        appendStrTab(strTab, " * Incomplete due to an %s.\n",
                     (p->flg & PROC_BADINST) ? "untranslated opcode" : "indirect JMP");
    if (p->flg & PROC_ICALL)
        appendStrTab(strTab, " * Indirect call procedure.\n");
    if (p->flg & IMPURE)
        appendStrTab(strTab, " * Contains impure code.\n");
    if (p->flg & NOT_HLL)
        appendStrTab(strTab, " * Contains instructions not normally used by compilers.\n");
    if (p->flg & FLOAT_OP)
