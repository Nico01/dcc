...
int scan(uint32_t ip, PICODE p)
{
    ...
    if (p->ic.ll.opcode) { // Save bytes of image used
...
static void setAddress(int i, bool fdst, uint16_t seg, int16_t reg, uint16_t off)
{
    ...
    PMEM pm = (!(stateTable[i].flg & TO_REG) == fdst) ? &pIcode->ic.ll.dst : &pIcode->ic.ll.src;

    /* Set segment. A later procedure (lookupAddr in proclist.c) will provide
       the value of this segment in the field segValue. */
...
static void rm(int i)
{
    ...
    if ((stateTable[i].flg & NSP) &&
        (pIcode->ic.ll.src.regi == rSP || pIcode->ic.ll.dst.regi == rSP))
...
static void segop(int i) { setAddress(i, true, 0, (((int16_t)i & 0x18) >> 3) + rES, 0); }
...
// Implied AX source
static void axSrcIm(int i) { pIcode->ic.ll.src.regi = rAX; }


// Implied AL source
static void alImp(int i) { pIcode->ic.ll.src.regi = rAL; }


// memImp - Plugs implied src memory operand with any segment override
static void memImp(int i) { setAddress(i, false, SegPrefix, 0, 0); }
...
{
    static const llIcode immedTable[8] = { iADD, iOR, iADC, iSBB, iAND, iSUB, iXOR, iCMP };
    static const uint8_t uf[8] = { 0, 0, Cf, Cf, 0, 0, 0, 0 };
...
{
    static const llIcode shiftTable[8] = { iROL, iROR, iRCL, iRCR, iSHL, iSHR, 0, iSAR };
    static const uint8_t uf[8] = { 0, 0, Cf, Cf, 0, 0, 0, 0 };
    static const uint8_t df[8] = { Cf, Cf, Cf, Cf, Sf | Zf | Cf, Sf | Zf | Cf, 0, Sf | Zf | Cf };
...
{
    static const llIcode transTable[8] = { iINC, iDEC, iCALL, iCALLF, iJMP, iJMPF, iPUSH, 0 };
    static const uint8_t df[8] = { Sf | Zf, Sf | Zf, 0, 0, 0, 0, 0, 0 };
...
{
    static const llIcode arithTable[8] = { iTEST, 0, iNOT, iNEG, iMUL, iIMUL, iDIV, iIDIV };
    static const uint8_t df[8] = { Sf | Zf | Cf, 0, 0, Sf | Zf | Cf, Sf | Zf | Cf, Sf | Zf | Cf,
                                   Sf | Zf | Cf, Sf | Zf | Cf };

    uint8_t opcode = pIcode->ic.ll.opcode = arithTable[REG(*pInst)];
    pIcode->ic.ll.flagDU.d = df[REG(*pInst)];
    rm(i);
...
{
    pIcode->ic.ll.immed.op = (stateTable[i].flg & S) ? signex(*pInst++) : *pInst++;
    pIcode->ic.ll.flg |= I;
}
...
static void data2(int i)
{
    ...
    /* ENTER is a special case, it does not take a destination operand,
       but this field is being used as the number of bytes to allocate on the stack. 
...
static void dispM(int i) { setAddress(i, false, SegPrefix, 0, getWord()); }


// dispN - 2 byte disp as immed relative to ip
static void dispN(int i)
{
    long off = getWord(); // Signed displacement

    pIcode->ic.ll.immed.op = (uint32_t)(off + (pInst - prog.Image));
    pIcode->ic.ll.flg |= I;
}
...
static void dispF(int i)
{
    ...
    pIcode->ic.ll.immed.op = off + ((uint32_t)seg << 4);
    pIcode->ic.ll.flg |= I;
}
...
static void strop(int i)
{
    if (RepPrefix) {
        pIcode->ic.ll.opcode +=
            ((pIcode->ic.ll.opcode == iCMPS || pIcode->ic.ll.opcode == iSCAS) && RepPrefix == iREPE)
                ? 2
                : 1;
...
{
    pIcode->ic.ll.immed.op = REG(*pInst) + (uint32_t)((i & 7) << 3);
    pIcode->ic.ll.flg |= I;
    rm(i);
}
...
{
    uint16_t wOp = pIcode->ic.ll.immed.op;
