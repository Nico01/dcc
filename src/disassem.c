...
static char *szFlops0C[] = { "FCHS", "FABS", "???", "???", "FTST", "FXAM", "???", "???" };
...
static char *szFlops1C[] = { "???", "???", "FCLEX", "FINIT", "FTST", "FXAM", "???", "???" };
...
void disassem(int pass, PPROC ppProc)
{
    ...
    if (pass != 3) { // Open the output file (.a1 or .a2 only)
    ...
    pc = memcpy(allocMem(cb), pProc->Icode.icode, (size_t)cb); // Create temporary code array
    ...
    pl = memset(allocMem(numIcode * sizeof(int)), 0, (size_t)(numIcode * sizeof(int)));

    if (pass != 3) // Write procedure header
        fprintf(fp, "\t\t%s  PROC  %s\n", pProc->name, (pProc->flg & PROC_FAR) ? "FAR" : "NEAR");

    for (int i = nextInst = 0; i < numIcode; i++) { // Loop over array printing each record
        dis1Line(i, false, 0, pass);
    }

    if (pass != 3) { // Write procedure epilogue
        fprintf(fp, "\n\t\t%s  ENDP\n\n", pProc->name);
        fclose(fp);
    }
...
*/
static void dis1Line(int i, bool fWindow, chtype attr, int pass)
{
    PICODE pIcode = &pc[i];
...
uint32_t pcTop;  // Image offset of top line
int icTop;       // Icode index  of top line
uint32_t pcCur;  // Image offset of cursor
int icCur;       // Icode index  of cursor
uint32_t pcBot;  // Image offset of bottom line
int icBot;       // Icode index  of bottom line
uint32_t pcLast; // Image offset of last instr in proc
...
void updateScr(bool new)
{
    ...
    for (y = 1, ic = icTop; y < LINES - 1; ic++) {
        ...
        if (ic == numIcode - 1) {
            ...
            default:
                // We have other than a break of control flow instruction at the end of the proc.
                // Parse more instructions to complete the basic block
                if ((ic = checkScanned(nextInst)) == -1) { //Some error.
...
/* Pop the current position on the position stack
...
/*
   ...
   Scan it if necessary, adjusting the allocation of pc[] and pl[] if necessary.
   Returns -1 if an error, otherwise the icode offset
*/
static int checkScanned(uint32_t pcCur)
{
    int i;
    ...
    if (pcCur >= (uint32_t)prog.cbImage) // Couldn't be!
        return -1;
    ...
    if (!labelSrch(pc, numIcode, pcCur, &i)) {
        ...
        if (numIcode >= allocIcode) {
            ...
            // It is important to clear the new icodes to ensure that the type is set to NOT_SCANNED
    ...
    if (pc[i].type == NOT_SCANNED) {
        ...
        if (scan(pcCur, &pc[i]) == IP_OUT_OF_RANGE) // Something went wrong... just forget it
            return -1;
    }
...
/*
   ...
   This includes some important initialisations, allocations, etc
   that are normally done in disassem()
*/
static void setProc(PPROC proc)
{
...
void interactDis(PPROC initProc, int initIC)
{
    ...
    while (fInteract) {
            ...
            if (pcCur >= pcBot) {
                ...
                for (int j = 0; j < NSCROLL; j++) {
                    ...
                    if (labelSrch(pc, numIcode, pcTop, &i))
                        ...
                        break; // Some problem... no more scroll
                }
            }
            updateScr(false);
            break;
        ...
        case KEY_PPAGE:
            ...
            if (i >= numIcode) // Something went wrong. Goto to first icode
                i = 0;
        ...
        case 'l': // Add a symbolic label here
        {
            ...
            if (strlen(cbuf) >= SYMLEN) // Name too long. Truncate
                cbuf[SYMLEN - 1] = '\0';
...
static void dispData(uint16_t dataSeg)
{
    ...
    if (pcCur >= (uint32_t)prog.cbImage) // We're at an invalid address. Use 0x100 instead
        pcCur = 0;
    ...
    for (int y = 1; y < LINES - 1;) {
        mvprintw(y, 1, "%04lX ", pc);
        for (int i = 0; i < 16; i++) {
            printw("%02X ", prog.Image[pc++ + off]);
            if ((pc + off) > prog.cbImage)
                break;
        }
        pc = pcStart;

        for (int i = 0; i < 16; i++) {
            int c = prog.Image[pc++ + off];
            if ((c < 0x20) || (c > 0x7E))
                c = '.';
...
bool callArg(uint16_t off, char *sym)
{
    PPROC p, pPrev;
    ...
    if (p == 0) { // No existing proc entry
        LibCheck(p);
        if (p->flg & PROC_ISLIB) {
            // No entry for this proc, but it is a library function. Create an entry for it
    ...
    if (p) { // We have a proc entry for this procedure. Copy the name
        strcpy(sym, p->name);
        return true;
    }
...
static void flops(PICODE pIcode)
{
    ...
    if ((pIcode->ic.ll.dst.regi == 0) || (pIcode->ic.ll.dst.regi >= INDEXBASE)) {
        ...
        /* The mod/rm mod bits are set to 11 (i.e. register).
           Could be specials (0x0C-0x0F, etc), or the st(i) versions of normal opcodes.
           Because the opcodes are slightly different for this case
           (e.g. op=04 means FSUB if reg != 3, but FSUBR for reg == 3),
