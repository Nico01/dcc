 ...
 *		instruction "JCOND (reg != 0) labX" because we do not know yet if
...
/* Sets up the TARGET flag for jump target addresses, finds idioms, and
...
{ Int i, j;                 /* idx into icode array */
  ...
  dword *p;                 /* for case table       */
    ...
    for (i = 0; i < pProc->Icode.numIcode; i++)
        ...
        {
            if (labelSrch(pIcode, pProc->Icode.numIcode, 
                                            pIcode[i].ic.ll.immed.op, &j))
